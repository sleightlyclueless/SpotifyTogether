...
router.get("/login_response", async (req, res) => {
  ...
  if (state === null)
    ...
    console.log("Redirect: " + DI.spotifyRedirectUri);
      ...
      .then((tokenResponse) => {
        axios
          ...
          .then(async (userResponse) => {
            let user = await DI.em.findOne(User, {spotifyId: userResponse.data.id});
            if (user) {
              // update user tokens
              ...
              res.redirect(`${DI.frontendUrl}/?userid=${user.userid}`);
            } else {
              // create new user
